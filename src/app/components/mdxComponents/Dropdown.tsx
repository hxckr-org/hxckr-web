    ...
    <div className="p-5 border-b border-b-grey-accent cursor-pointer">
      ...
      >
        <section className="flex items-center gap-2">
          <p
            className={`${
              isOpen ? "text-purple-primary" : "text-black"
          ...
          {selectedItem && (
            <span className="flex items-center gap-2">
              ...
              <p className="capitalize text-green-primary text-base font-bold italic font-p22mackinac">
                {selectedItem}
              </p>
            </span>
          )}
      ...
      {isOpen && (
        <section className="pt-2 flex flex-col gap-4">
          <p className="text-sm font-light text-grey-secondary-text">
            {content}
          </p>
          <div className="flex gap-3">
            {(dropDownChildrenArray as React.ReactElement[]).map(
              (item, idx) => (
                <button
                  key={idx}
                  onClick={() =>
