...
const NavItem = ({ href, icon, label, isActive, isOpen }: NavItemProps) => (
  <Link
    href={href}
...
export default function Sidebar({
  ...
  return (
    <aside
      className={`flex h-screen flex-col border-r bg-white ${
        !isOpen ? "w-[104px]" : "w-[266px]"
      }`}
    >
      <div
        className={`relative flex items-center h-24 justify-between border-b border-grey-button-border`}
      >
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="absolute -right-5 p-3 rounded-full border border-grey-button-border bg-white"
        >
          <DoubleChevronLeft
