...
export const ChallengeCard = ({ challenge }: { challenge: ChallengeWithProgress }) => {
  ...
  return (
    <div className='flex flex-col justify-between overflow-hidden rounded-lg min-w-[357px] max-w-[400px] h-[470px] border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md'>
      <div className='relative px-4 pt-4'>
        ...
        <span
          className={`${
            challenge.progress.status.toLowerCase() === "inprogress" ? "block" : "hidden"
      ...
      <div className='p-4'>
        <h3 className='text-lg font-semibold text-gray-900 capitalize'>{challenge.title}</h3>
        ...
        <div className='mt-4'>
          <div className='mb-2 flex items-center justify-between text-sm text-gray-600'>
            <span className='text-[#888888] text-sm font-light'>
              <span className='text-black'>{challenge.progress.current_step}/</span>
              {challenge.module_count} tasks
            </span>
            <span className='text-[#888888] text-sm font-light'>
              {Math.min(100, Math.round((challenge.progress.current_step * 100) / Math.max(1, challenge.module_count)))}% Done
            </span>
          </div>
          <div className='h-2 w-full overflow-hidden rounded-full bg-[#F8F2FF]'>
            <div
              ...
              style={{
                width: `${Math.min(100, (challenge.progress.current_step * 100) / Math.max(1, challenge.module_count))}%`,
              }}
            />
          </div>
        </div>
        <Link
          href={`/challenges${challengeKey}?rid=${challenge.repository_id}`}
          // href={`/challenges/${challenge.title.toLowerCase().replaceAll(" ", "-")}?rid=${challenge.repository_id}`}
          onClick={() => {
        ...
        >
          {challenge.progress.status.toLowerCase() === "inprogress" ? "Continue" : "Start"}
          <ArrowLeftIcon className='h-4 w-4 rotate-180 group-hover:translate-x-1 transition-transform duration-300' fill='#9747FF' />
        </Link>
      </div>
    </div>
  );
